...
class Generator:
    ...
    def createHoneyccrBSdeployment(self, radius, numberOfBS = 36, omnidirectionalAntennas = False, SFR = False):
        ...
        if x*y < numberOfBS:
            y += 1
        print("Network has",x,"columns and",y,"rows")
        ...
        for i in range(numberOfBS):
            ...
            for j in range(sectors):
                ...
                bs.Rc = radius
                bs.angle = 120 * j
                even_row_offset =  (1-int(i%(2*x)/x)) * math.sqrt(3)/2 * radius
                print("i=",i,"ero=",even_row_offset)
